...
void writeHuffmanToFile(FILE *file, HalfASCIIHuffman *huffman) {
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        fwrite(&huffman->symbols[i].symbol, sizeof(huffman->symbols[0].symbol), 1, file);
 ...
 *      Byte 8:43     The huffman table used to compress the file. The symbols have variable length so after every
 ...
 *      Byte 44:end   The compressed data
 *
...
void compressFile(FILE *file, char *filename, HalfASCIIHuffman *huffman, uint16_t blockSize) {
    ...
    for (int i = 0; i < file_len; ++i) {
        ...
        symbol = huffman->symbols[half_char].symbol;  // The symbol of the read char
        symbol_length = huffman->symbols[half_char].symbol_length;  // The number of bits of the symbol

        for (int j = 0; j < 2; ++j) {
            if (write_index + 1 - symbol_length < 0) {  // If the buffer can't fit the symbol
                // The buffer can fit write_index + 1 bits of the symbol

                uint16_t mask = (1 << (symbol_length - write_index - 1)) - 1;
                ...
                updateBuffer(buffer, &buff_index, &write_index, &nBlocks, compressed, remaining_length,
                             remaining_symbol, blockSize);
            ...
            // get the second half of the character
            half_char = c & lsb_mask;
...
void decompressFile(char *filename){
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        fread(&huffman.symbols[i].symbol, sizeof(huffman.symbols[0].symbol), 1, file);
        fread(&huffman.symbols[i].symbol_length, sizeof(huffman.symbols[0].symbol_length), 1, file);
    }
